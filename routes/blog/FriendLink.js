...
router.post('/applyFriendLink', async (ctx, next) => {
    const {name, website, icon, email, container, userid, username, ip } = ctx.request.body
    ctx.response.status = 200
    let insertsql = ` insert into friendlink(name,website,icon,email,container,userid,username,userip) value(${SqlString.escape(name)},'${website}',${SqlString.escape(icon)},${SqlString.escape(email)},${SqlString.escape(container)},${SqlString.escape(userid)},${SqlString.escape(username)},${ip}) `
    try {
